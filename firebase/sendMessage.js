...
var sendNotifForSeries = function sendNotifForSeries(series_id, uid) ***REMOVED***
    request(
        ...
        function(latest_error, latest_response, latest_body) ***REMOVED***
            if (latest_body == "404 -- Not Found") return "failed";
            latest_body = JSON.parse(latest_body);
            var job = new CronJob(
                new Date(Date.now() + 5000), //time of start
                () => ***REMOVED***
                    request(
                            ...
                            userRef
                                ...
                                .then(snapshot => ***REMOVED***
                                    snapshot.forEach(doc => ***REMOVED***
                                        ...
                                        sendMessage(***REMOVED***
                                            title: `📺$***REMOVED***
                                                info_body.seriesName
                                            ***REMOVED***📺`,
                                            body: `ℹ️"$***REMOVED***
                                                latest_body.episodeName
                                            ***REMOVED***" is now airing!`,
                                            token: token,
                                            priority: "high"
                                        ***REMOVED***);
                                        return true;
