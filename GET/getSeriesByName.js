...
router.get('/', function (req, res) { // Hier wird definiert das dieser Endpunkt GET benutzen soll. Dann wird eine Methode angegeben , die jedes Mal ausgeführt werden soll, wenn der Enpunkt  kontaktiert wird. Dabei stehen die Parameter res und req jeweils für rquest (zu dt. Anfrage) und res (zu dt. Antwort)
	let queryString = req.query;
	let lang = req.query.lang || 'en'; //Variable lang ist entweder der Inhalt des Query Strings oder standardmäßig 'en' 
	let tvdb = require(`${__dirname}/../tvdb_${lang}.js`); //entsprechend der Variable lang wird das tvdb.js Modul importiert.
	tvdb //Nun wird mithilfe des API wrappers (importiert aus tvdb.js eine Anfrage an die API geschickt
		...
		.catch(error => { //Ein potentieller Fehler wird gefangen (korrekte Fachsparche hierzu auf Deutsch zu finden erwies sich als unmöglich.)
